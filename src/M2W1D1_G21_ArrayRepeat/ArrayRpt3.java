...
public class ArrayRpt3 {
    public static void main(String[] args) {
        ...
        int min=mat[0][0];
        for (int satir = 0; satir <mat.length ; satir++) {

            for (int sutun = 0; sutun <mat[satir].length ; sutun++) {

                if(min>mat[satir][sutun])
                   min=mat[satir][sutun];
      ...
      //  2D arrayinden min number print et
        ...
        {
            for(int j=0;j<mat[i].length ; j++) // i: SATIR---j::sutun****
            {
                 if (mat[i][j] < min)
                     min=mat[i][j];
            }
